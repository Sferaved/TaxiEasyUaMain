...
public class CustomListFinishAdapter extends BaseAdapter {

    private final Context context;
    private final String[] dataList;
    private final int numItemsToShow; // Количество элементов, которые вы хотите показать

    public CustomListFinishAdapter(Context context, String[] dataList, int numItemsToShow) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.services_adapter_finish_layout, parent, false);
        }
