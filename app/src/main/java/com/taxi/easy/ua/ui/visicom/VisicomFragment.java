...
import com.taxi.easy.ua.utils.location.GpsButtonAppearanceHelper;
...
//        setupImages();
        ...
        permissionLauncher = registerForActivityResult(
                ...
                result -> {
                    ...
                    boolean xStatus = viewModel != null && Boolean.TRUE.equals(viewModel.getStatusX().getValue());
...
//         Отключаем GNSS проверку на эмуляторе
    ...
    public static void updateGpsButtonCross(boolean show) {
        if (gpsBtn != null && getCurrentActivity() != null) {
            ...
            GpsButtonAppearanceHelper.Appearance appearance =
                    GpsButtonAppearanceHelper.resolve(show, gpsEnabled, hasFineLocationPermission);
            Logger.d(null, TAG, "updateGpsButtonCross: show=" + show
...
//        }
        ...
        if (urlAPI.equals("orderClientCostMyApi")) {
            ...
            if ("google_pay_payment".equals(payment_type) && frozenSubmitAddCost >= 0) {
                ...
                if (frozenTotal > 0 && startCost > 0 && frozenTotal >= startCost) {
                    addCostInt = frozenTotal - startCost;
                }
            }
...
//                addCost = addCostBlackList(addCost);
            ...
            if ("google_pay_payment".equals(payment_type)
                    && CostParseHelper.hasDisplayableCost(pendingOrderDisplayCost)) {
                clientCost = CostParseHelper.normalizeCostString(pendingOrderDisplayCost);
            }
...
// Обновляем ViewModel, но НЕ вызываем updateGpsButtonCross повторно
    ...
    private void gpsButSetOnClickListener(LocationManager locationManager) {
        if (locationManager != null) {
            if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                boolean hasLocationPermission = hasUsableLocationPermission();
                location_update = hasLocationPermission;

                if (!hasLocationPermission) {
                    sharedPreferencesHelperMain.saveValue("setStatusX", true);
                    viewModel.setStatusX(true);
                    if (shouldPromptSystemLocationPermission()) {
                        checkPermission();
                    } else {
                        MyBottomSheetGPSFragment bottomSheetDialogFragment =
                                new MyBottomSheetGPSFragment(getString(R.string.location_on));
                        bottomSheetDialogFragment.show(getChildFragmentManager(), bottomSheetDialogFragment.getTag());
                    }
                    updateGpsButtonCross(true);
                    return;
    ...
    private boolean hasUsableLocationPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
    ...
    /** Системный диалог, пока Android ещё может его показать; иначе — переход в настройки. */
    private boolean shouldPromptSystemLocationPermission() {
        if (loadPermissionRequestCount() < 3) {
            return true;
        }
        Activity activity = getActivity();
        if (activity == null) {
...
//                        createBlackList();
    ...
    private boolean isGooglePaySubmitFrozen() {
        return googlePayOrderHoldInProgress || EarlyOrderNavigationHelper.isSubmitInProgress();
    }

    private void initAddCostOnCreateView() {
        if (!isGooglePaySubmitFrozen()) {
            addCost = 0;
            updateAddCost(String.valueOf(addCost), context);
            return;
        }
        Logger.d(context, TAG, "onCreateView: preserve addCost during GPay/submit");
        List<String> settings = logCursor(MainActivity.TABLE_SETTINGS_INFO, context);
        if (settings.size() > 5 && !settings.get(5).isEmpty()) {
            try {
                addCost = Long.parseLong(settings.get(5));
            } catch (NumberFormatException ignored) {
                addCost = 0;
            }
        }
        if (startCost > 0) {
            finalCost = startCost + addCost;
            if (text_view_cost != null) {
                text_view_cost.setText(String.valueOf(finalCost));
            }
        } else if (CostParseHelper.hasDisplayableCost(pendingOrderDisplayCost)) {
            finalCost = GooglePayOrderHelper.parseAmountUah(
                    CostParseHelper.normalizeCostString(pendingOrderDisplayCost));
        }
    }

    /** После возврата из GPay onCreateView мог обнулить addCost — восстанавливаем сумму заказа. */
    private void restoreFrozenGooglePayOrderCostForSubmit() {
        String frozen = null;
        if (CostParseHelper.hasDisplayableCost(pendingOrderDisplayCost)) {
            frozen = CostParseHelper.normalizeCostString(pendingOrderDisplayCost);
        } else if (CostParseHelper.hasDisplayableCost(pendingGooglePayAmount)) {
            frozen = CostParseHelper.normalizeCostString(pendingGooglePayAmount);
        }
        if (frozen == null) {
            return;
        }
        int totalUah = GooglePayOrderHelper.parseAmountUah(frozen);
        if (totalUah <= 0) {
            return;
        }
        finalCost = totalUah;
